...
public class CameraMovementScript : MonoBehaviour
{
    ...
    private void ZoomScroll() {
        ...
        if (!userSettings.invertScroll) {
            ...
            // inverted scroll, so -1
            invertScroll = -1;
        }
        // loop until i >= invertScroll (-1 if inverted, 1 if normal) * scroll input (which is usually 0.1 or 0.2)
        if (scrollInput > 0) {
            // zoom in scroll input * 10 num times
            for (int i = 0; i < invertScroll*scrollInput*10; i++) {
                ZoomIn();
            }
        }
